...
import { FiX, FiTrash2, FiUploadCloud, FiPlus, FiCalendar } from 'react-icons/fi';
...
export default function DateAutomationModal({ isOpen, onClose, onSaved, editAutomation }: Props) {
    ...
    useEffect(() => {
        if (editAutomation) {
            reset({
                ...
                audienceId: editAutomation.audienceId || '',
                attachment: null,
            });
    ...
    // Keep scheduledDates in sync with form
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        Object.entries({
            ...
            scheduledDates: JSON.stringify(finalDates),
        }).forEach(([k, v]) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-3xl max-h-[92vh] overflow-y-auto">
                <div className="flex items-center justify-between p-5 border-b">
                    <div>
                        <h2 className="text-xl font-bold flex items-center gap-2"><FiCalendar /> {editAutomation ? 'Edit Date Automation' : 'New Date-Based Automation'}</h2>
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-5">
                    ...
                    <div>
                        ...
                        <input {...register('name', { required: true })} className="w-full border rounded-lg px-3 py-2 text-sm" placeholder="e.g. Monthly Newsletter" />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">To Email <span className="text-gray-400 font-normal">(CC/BCC)</span></label>
                            <input {...register('toEmail')} className="w-full border rounded-lg px-3 py-2 text-sm" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Reply-To</label>
                            <input {...register('replyToEmail')} className="w-full border rounded-lg px-3 py-2 text-sm" />
                        </div>
                    </div>
